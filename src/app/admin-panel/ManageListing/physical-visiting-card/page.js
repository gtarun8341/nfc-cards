...
const AdminPhysicalVisitingCardPage = () => {
  ...
  const fetchTemplates = async () => {
    try {
      ...
      const response = await api.get(
        `/api/templates/admin/templates?type=physical-visiting-card&page=${page}&limit=9`,
        config
      );
  ...
  const previewTemplate = async (templateId) => {
    try {
      ...
      const response = await api.get(
        `/api/templates/admin/render/${templateId}?type=physical-visiting-card`,
        config
      );
  ...
  const handleDeleteTemplate = async (templateId, usageCount) => {
    try {
      ...
      await api.delete(
        `/api/templates/${templateId}?type=physical-visiting-card`,
        config
      );
  ...
  return (
    <div className="container mx-auto p-6">
      <h1 className="text-2xl font-bold mb-4">
        Admin - Manage physical-visiting-card Templates
      </h1>
