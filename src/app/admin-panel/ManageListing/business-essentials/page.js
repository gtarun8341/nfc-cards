...
const AdminBusinessEssentialsPage = () => {
  ...
  const fetchTemplates = async () => {
    try {
      ...
      const response = await api.get(
        `/api/templates/admin/templates?type=business-essentials&page=${page}&limit=9`,
        config
      );
  ...
  const previewTemplate = async (templateId) => {
    try {
      ...
      const response = await api.get(
        `/api/templates/admin/render/${templateId}?type=business-essentials`,
        config
      );
  ...
  const handleDeleteTemplate = async (templateId) => {
    try {
      ...
      await api.delete(
        `/api/templates/${templateId}?type=business-essentials`,
        config
      );
  ...
  return (
    <div className="container mx-auto p-6">
      <h1 className="text-2xl font-bold mb-4">
        Admin - Manage Business Essentials Templates
      </h1>
