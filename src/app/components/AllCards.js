      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6"> {/* Change to 3 columns on large screens */}
        {cardsData.map((card) => (
          <div key={card.id} className="border rounded-lg p-4 shadow-lg">
            ...
            <p className={`text-gray-600 ${full ? 'text-left' : 'text-center'}`}>{card.description}</p> {/* Left align description if full */}
            {full && (
              <button className="mt-4 bg-blue-500 text-white py-2 px-4 rounded">Select</button> // Select button
            )}
          </div>
        ))}
      </div>
