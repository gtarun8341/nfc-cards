...
const MiniWebsitePage = () => {
  ...
  const fetchStats = async () => {
    try {
      ...
      const res = await api.get(
        `/api/selectedtemplates/template-stats?type=mini-website`,
        config
      );
  ...
  const fetchTemplates = async (page = 1) => {
    try {
      ...
      const response = await api.get(
        `/api/templates/templates?type=mini-website&page=${page}&limit=9`,
        config
      );
      ...
      const selectedTemplatesResponse = await api.get(
        "/api/selectedtemplates/link?type=mini-website",
        config
      );
  ...
  const previewTemplate = async (templateId) => {
    try {
      ...
      const response = await api.get(
        `/api/templates/render/${templateId}?type=mini-website`,
        config
      );
  ...
  const handleSelectTemplate = async (templateId) => {
    try {
      ...
      const response = await api.post(
        "/api/selectedtemplates/select?type=mini-website",
        { templateId },
        config
      );
      setSelectedTemplateData({
  ...
  const handleDeleteSelectedTemplate = async () => {
    try {
      ...
      await api.post(
        "/api/selectedtemplates/delete?type=mini-website",
        {},
        config
      );
  ...
  return (
    <div className="container mx-auto p-6">
      <h1 className="text-3xl font-bold text-center mb-6 text-blue-600">
        Available Mini Website Templates
      </h1>
