...
const PDFVisitingCardPage = () => {
  ...
  const fetchStats = async () => {
    try {
      ...
      const res = await api.get(
        `/api/selectedtemplates/template-stats?type=pdf`,
        config
      );
  ...
  const fetchTemplates = async () => {
    try {
      ...
      const response = await api.get(
        `/api/templates/templates?type=pdf&page=${page}&limit=9`,
        config
      );
      ...
      const selectedTemplatesResponse = await api.get(
        "/api/selectedtemplates/link?type=pdf",
        config
      );
      console.log(
  ...
  const previewTemplate = async (templateId) => {
    try {
      ...
      const response = await api.get(
        `/api/templates/render/${templateId}?type=pdf`,
        config
      );
  ...
  const handleSelectTemplate = async (templateId) => {
    try {
      ...
      const response = await api.post(
        "/api/selectedtemplates/select?type=pdf",
        { templateId },
        config
      );
      setSelectedTemplateData({
  ...
  const handleDeleteSelectedTemplate = async () => {
    try {
      ...
      await api.post("/api/selectedtemplates/delete?type=pdf", {}, config);
  ...
  return (
    <div className="container mx-auto p-6">
      <h1 className="text-3xl font-bold text-center mb-6 text-blue-600">
        Available PDF Visting Card Templates
      </h1>
