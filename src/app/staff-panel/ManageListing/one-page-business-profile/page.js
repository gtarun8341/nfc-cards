...
const AdminOnePageBusinessProfilePage = () => {
  ...
  const fetchTemplates = async () => {
    try {
      ...
      const response = await api.get(
        `/api/templates/staff/templates?type=one-page-business-profile&page=${page}&limit=9`,
        config
      );
  ...
  const previewTemplate = async (templateId) => {
    try {
      ...
      const response = await api.get(
        `/api/templates/staff/render/${templateId}?type=one-page-business-profile`,
        config
      );
  ...
  return (
    <div className="container mx-auto p-6">
      <h1 className="text-2xl font-bold mb-4">
        Staff - Manage one-page-business-profile Templates
      </h1>
