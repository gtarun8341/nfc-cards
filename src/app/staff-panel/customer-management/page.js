...
const CustomerManagementPage = () => {
  ...
  useEffect(() => {
    const fetchUsersWithPlans = async () => {
      try {
        ...
        const response = await api.get('/api/user-plans/staff/active-plans', config);
  ...
  const filteredUsers = usersWithPlans.filter((user) => {
    ...
    const matchesTitle =
      filterByTitle === "" || (user.plan && user.plan.title.toLowerCase().includes(filterByTitle.toLowerCase()));

    const matchesExpiry =
      filterByExpiry === "" ||
      (user.plan &&
        new Date(user.plan.expiryDate).toLocaleDateString().includes(filterByExpiry));
